...
const repository = __importStar(require("../repository/report-income-cost.repository"));
...
const All = async (req) => {
    ...
    try {
        ...
        for (let index = 0; index < 12; index++) {
            years.push((0, moment_1.default)(String(date), "YYYY-MM-DD")
                .startOf("year")
                .add(index, "M")
                .format("YYYY-MM-DD"));
        }
        let triwulan = await repository.getTriwulan(triwulanDate);
        let year = await repository.getTriwulan(years);
        for (const data of triwulan) {
            if (data.filename) {
                data.filename = data.filename.split(",");
            }
        }
        for (const data of year) {
            if (data.filename) {
                data.filename = data.filename.split(",");
            }
        }
...
const getDetailFile = async (req) => {
    ...
    try {
        // let { code } = req.query;
        const codeC = req.query.code;
        const code = codeC ? String(codeC).replace(/\u00A0/g, '').replace(/\s+/g, '').replace(/\\n/g, '') : '';
        let date = (0, moment_1.default)(String(codeC), "YYYY_MM").format("YYYY_MM");
        let triwulanDate = [];
        let year = [];
...
const saveDetailFile = async (req) => {
    let response;
    try {
        let { no_dok, deskripsi, nominal } = req.body;
        const codeC = req.body.code;
        const code = codeC ? String(codeC).replace(/\u00A0/g, '').replace(/\s+/g, '').replace(/\\n/g, '') : '';
        let file;
        let filename = "";
        let pathFile = "";
