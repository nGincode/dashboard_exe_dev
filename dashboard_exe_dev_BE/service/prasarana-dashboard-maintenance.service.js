...
exports.add = exports.getAll = exports.getMaintenance = void 0;
const repository = __importStar(require("../repository/prasarana-dashboard-maintenance.repository"));
...
exports.add = add;
const getMaintenance = async (periodMonth, periodeYear) => {
    ...
    try {
        ...
        const data = await repository.getIndicatorMaintenance(stringPeriod);
        ...
        const queryFunctions = [
            ...
            },
            {
                ...
                process: (technical, indicator) => {
                    ...
                    if (techData) {
                        const totalRealization = Number(techData.i_value_1) +
                            Number(techData.i_value_2) +
                            Number(techData.i_value_3);
                        indicator.realization = totalRealization;
                        indicator.percentage = Number(((totalRealization / indicator.target) * 100).toFixed(2));
                    }
                },
        ...
        let dataSertifikasiPerawatanHarus = await repository.getIndicatorSertifikasiHarus('PERAWATAN', periodMonth, periodeYear);
        let dataSertifikasiPerawatan = await repository.getIndicatorSertifikasi('PERAWATAN', periodMonth, periodeYear);
        let totalSertifikasiPerawatan = Number(((dataSertifikasiPerawatan.rows[0].count / dataSertifikasiPerawatanHarus.rows[0].count) * 100).toFixed(2));
        totalSertifikasiPerawatan = (isNaN(totalSertifikasiPerawatan) || !isFinite(totalSertifikasiPerawatan)) ? 0 : totalSertifikasiPerawatan;
        let recordSertifikasi = [
            ...
            },
            {
                ...
                "order": 2,
                "target": 100,
                "indicator": "Jumlah tenaga perawat yang memiliki sertifikat keahlian",
                "percentage": Number((totalSertifikasiPerawatan / 100 * 100).toFixed(2)),
                "c_indicator": "",
                "realization": totalSertifikasiPerawatan
            }
        ];
        indicators.push({
        ...
        let dataPelaporan = await repository.getIndicatorPelaporan(stringPeriod, totalSertifikasiPerawatan);
        indicators.push({
            'parameter': 'Pelaporan',
            'record': [
                {
                    ...
                    "order": 1,
                    "target": 100,
                    "indicator": "Kepatuhan ketepatan waktu pelaporan perawatan",
...
function responseDataJson(result, periodMonth, periodeYear) {
    ...
    if (record.length !== 0) {
        data.push({ parameter: parameter, record });
    }
