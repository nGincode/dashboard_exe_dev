...
const repository = __importStar(require("../repository/prasarana-operation.repository"));
...
const getOperationSafety = async (periodeYear, periodeMonth, status) => {
    let response;
    try {
        const result = await repository.getOperationSafety(periodeYear, periodeMonth, status);
        const dataResponse = convertToResponseOperationSafety(result.rows);
        if (result) {
...
const saveOperationSafety = async (req) => {
    let response;
    try {
        const periodeYear = req.body.period_year;
        const periodMonth = req.body.period_month;
        const data = req.body.data;
        const result = await repository.saveOperationSafety(req, periodeYear, periodMonth, data, req.userId);
        if (result) {
...
const getOperationOperational = async (periodMonth, periodYear, i_status_active) => {
    ...
    try {
        const data = await repository.getOperationOperational(statusActive, stringPeriod);
        if (!data) {
            response = build.response('400', 'invalid input type', []);
        }
        else {
...
const saveOperationOperational = async (request) => {
    ...
    try {
        ...
        await repository.saveOperationOperational(payload);
        request.body.menu = 'Prasarana Pengoperasian Operasional';
        request.body.description = 'Insert Data Operation Operational';
...
const getOperationTechnical = async (periodMonth, periodYear, statusActive) => {
    try {
        let results = { data: [], summary: [], summary_status: null };
        const stringPeriod = getPeriodDatesQuery(periodMonth, periodYear);
        const getData = await repository.getAllTechnical(stringPeriod, statusActive);
        const getSummary = await repository.getAllTechnicalSummary(stringPeriod);
        if (getData.rows.length > 0) {
            results.data = getData.rows.reduce((acc, row) => {
        ...
        if (getSummary.rows.length > 0) {
            results.summary = getSummary.rows.reduce((acc, row) => {
                const { n_indicator, ...rowValues } = row;
                const idx = acc.findIndex((val) => val.n_indicator === n_indicator);
                if (acc.length > 0 && idx >= 0)
                    acc[idx].record.push(rowValues);
                else
                    acc.push({ n_indicator: row.n_indicator, record: [rowValues] });
                return acc;
            }, []);
            results.summary_status = getSummary.rows[0].i_status_active;
        }
        return customResponse('00', 'success', results);
    }
    catch (error) {
...
const saveOperationTechnical = async (req) => {
    ...
    try {
        ...
        if (insertData.length > 0) {
            ...
            if (insert.rowCount < 0)
                throw new Error('Save Operation Technical Failed');
            logData.body.menu = "Prasarana Pengoperasian Teknis";
            logData.body.description = "Insert Data Pengoperasian Teknis";
            ...
            if (update.rowCount < 0)
                throw new Error('Update Operation Technical Failed');
            logData.body.menu = "Prasarana Pengoperasian Teknis";
            logData.body.description = "Update Data Pengoperasian Teknis";
...
const saveOperationTechnicalSummary = async (req) => {
    const userId = req.userId;
    const data = req.body;
    try {
        const { period_month: i_period_month, period_year: i_period_year, data: dataPerPeriod } = data;
        let insertData = [];
        let updateData = [];
        const inputData = dataPerPeriod.reduce((acc, { n_indicator, record }) => {
            record.forEach((recordValue) => acc.push({ n_indicator, i_period_month, i_period_year, userId, ...recordValue }));
            return acc;
        }, []);
        const oldData = await repository.getTechnicalSummaryByNamePeriod(inputData);
        inputData.forEach((val) => {
            if (oldData.rows.some(row => row.n_indicator === val.n_indicator))
                updateData.push(val);
            else
                insertData.push(val);
        });
        let logData = { ...req };
        if (insertData.length > 0) {
            const insert = await repository.saveOperationTechnicalSummary(insertData);
            if (insert.rowCount < 0)
                throw new Error('Save Operation Technical Summary Failed');
            logData.body.menu = "Prasarana Pengoperasian Teknis";
            logData.body.description = "Insert Data Pengoperasian Teknis Summary";
            logData.body.j_new_data = insert.rows;
            await (0, activity_service_1.InsertActivity)(logData);
        }
        else if (updateData.length > 0) {
            const update = await repository.updateOperationTechnicalSummary(updateData);
            if (update.rowCount < 0)
                throw new Error('Update Operation Technical Summary Failed');
            logData.body.menu = "Prasarana Pengoperasian Teknis";
            logData.body.description = "Update Data Pengoperasian Teknis Summary";
            logData.body.j_old_data = oldData.rows;
            logData.body.j_new_data = update.rows;
            await (0, activity_service_1.InsertActivity)(logData);
        }
...
const saveOperationTechnicalFile = async (req) => {
    try {
        const { i_id, d_opr } = req.body;
        const files = req.files?.n_file_path;
        if (!i_id)
            return build.response('400', 'ID Operation Technical is required!', []);
        if (!d_opr)
            return build.response('400', 'Date Opr. Operation Technical is required!', []);
        if (!files)
            return build.response('400', 'File Operation Technical is required!', []);
        const getRow = await repository.getTechnical(i_id);
        if (getRow.rows.length <= 0)
            return build.response('400', 'Operation Technical not found!', []);
        ...
        if (update?.rowCount < 0)
            throw new Error('Update Operation Technical File Failed');
        const logData = { ...req };
        logData.body.menu = "Prasarana Pengoperasian Teknis";
        logData.body.description = "Update File Pengoperasian Teknis";
...
const deleteOperationTechnicalFile = async (req) => {
    try {
        const { i_id } = req.body;
        if (!i_id)
            return build.response('400', 'ID Operation Technical is required!', []);
        ...
        if (deleted?.rowCount < 0)
            throw new Error('Delete File Operation Technical Failed');
        ...
        const logData = { ...req };
        logData.body.menu = "Prasarana Pengoperasian Teknis";
        logData.body.description = "Delete Data Pengoperasian Teknis";
        logData.body.j_old_data = getRow.rows;
        await (0, activity_service_1.InsertActivity)(logData);
        return build.response('00', 'success', {});
    }
...
const getBuildMerge = async (req) => {
    try {
        ...
        logData.body.menu = "Prasarana Pengoperasian Teknis";
        logData.body.description = "Merge PDF Data Pengoperasian Teknis";
        await (0, activity_service_1.InsertActivity)(logData);
        return build.response('00', 'success', {});
    }
    catch (error) {
...
const getFileProps = (n_indicator, date, files) => {
    ...
    const pathFile = `upload/origin/pengoperasian/${day[0]}/${day[1]}`;
    return { extension, filename, pathFile };
};
const getFile = async (periodDate) => {
    try {
        const pathFileMerge = `upload/merge/pengoperasian/${periodDate}`;
...
const mergeFile = async (periodDate) => {
    try {
        ...
        const pathFileOrigin = `upload/origin/pengoperasian/${periodDate}`;
        const pathFileMerge = `upload/merge/pengoperasian/${periodDate}`;
        ...
        const filename = `Pengoperasian Teknis_${periodDate.replace(/\//g, "")}_${dateNow}.pdf`;
...
const convertToResponseOperationSafety = (data) => {
    const dataResponse = [];
    for (let dateOperation of data) {
        ...
        const reasons = [{
                ...
                n_reason_3: dateOperation.n_reasons_3[0],
            }];
        dataResponse.push({
...
const customResponse = (code, message, data) => {
    if (code == "500")
        winston.logger.error(message, JSON.stringify(data));
    return {
        code: code,
        message: env_config_1.default.env().APP_FOR == "prod" && code == "500"
...
const getCertificate = async (periodeYear, periodMonth) => {
    ...
    // get data target certificate from struktur organisasi
    subDivisi = ['POKT', 'SCADA'];
    subUnit = 'PENGENDALI';
    targetCertificate = await repository.getTargetCertificate(periodMonth, periodeYear, subDivisi, subUnit);
    // get data realization certificate from sertifikasi
    realizationCertificate = await repository.getRealizationCertificate(periodMonth, periodeYear, subDivisi, subUnit);
    // process
    let record2 = [];
    for (const el of targetCertificate.rows) {
        let find = realizationCertificate.rows.find((e) => e.sub_kategori === el.sub_divisi);
...
const getReporting = async (periodeYear, periodMonth) => {
    ...
    // set sertifikasi
    let subDivisi = ['TRACK AND BRIDGE (M)', 'BUILDING (M)', 'THIRD RAIL (M)', 'POKT', 'SCADA'];
    let subUnit = 'PEMERIKSA';
