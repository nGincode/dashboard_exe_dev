...
exports.getIndicatorPelaporan = exports.getIndicatorSertifikasi = exports.getIndicatorSertifikasiHarus = exports.getIndicatorTechnical = exports.getIndicatorOperational = exports.getIndicatorSafety = exports.getTargetIndicator = exports.add = exports.getAll = exports.getIndicatorOperation = void 0;
...
const add = async (user_uuid, req, periodMonth, periodYear) => {
    ...
    try {
        await trx.raw(`SELECT * FROM pso.history_table('${periodMonth}', '${periodYear}', '${user_uuid}', null, 'dash_operation')`);
        const dataInsert = [];
        ...
        await trx('dash_operation').withSchema('prasarana').insert(dataInsert);
        trx.commit();
        return true;
    }
    catch (error) {
...
const getIndicatorSafety = async (stringPeriod) => {
    ...
    FROM prasarana.t_d_operation_safety
    WHERE ${stringPeriod}
...
const getIndicatorOperational = async (stringPeriod) => {
    ...
    FROM prasarana.t_d_operation_operational
    WHERE ${stringPeriod}
    AND i_status_active = 4
    GROUP BY c_indicator
  `;
...
const getIndicatorOperation = async (stringPeriod) => {
    const query = `
  ...
  `;
    try {
        ...
        if (error instanceof Error && error.code) {
            ...
            if (code === '22P02') {
                throw new Error(`Get Indicator Operation Failed: Invalid input syntax`);
            }
            else {
                throw new Error(`Get Indicator Operation Failed: Database error`);
            }
        }
        else {
            throw new Error('Get Indicator Operation Failed: Unexpected error occurred');
        }
    }
};
exports.getIndicatorOperation = getIndicatorOperation;
...
const getIndicatorPelaporan = async (stringPeriod, temp, temp2) => {
    let target = 7;
    let total = (temp > 0 || temp2 > 0) ? 2 : 0;
    let queryKeselamatan = `
    SELECT COUNT(i_id) FROM prasarana.t_d_operation_safety
    WHERE ${stringPeriod}
  ...
  `;
    ...
    SELECT COUNT(i_id) FROM prasarana.t_d_operation_operational
    WHERE ${stringPeriod}
    AND approved_by IS NOT NULL
  `;
    ...
    WHERE ${stringPeriod}
    AND c_dashboard_type = 'PENGOPERASIAN'
    AND c_param = 'TEKNIS'
  `;
    ...
    if (result.rows.length > 0) {
        for (const key of result.rows) {
            let query = `
        SELECT COUNT(i_id) FROM prasarana.t_d_operation_technical
        WHERE ${stringPeriod}
        AND approved_by IS NOT NULL
        AND c_indicator = ?
      `;
