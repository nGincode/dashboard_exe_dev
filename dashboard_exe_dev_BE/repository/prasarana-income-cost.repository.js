...
const Insert = async (data) => {
    let result = await db_config_1.default.withSchema("prasarana").table("pra_income_cost").insert(data);
    return result;
};
...
const Trx = async (month, year, userID, data) => {
    ...
    await db_config_1.default.transaction(async (trx) => {
        try {
            await trx.raw(`SELECT * FROM pso.history_table('${month}', '${year}', '${userID}', null, 'pra_income_cost')`);
            await trx.withSchema("prasarana").table("pra_income_cost").insert(data);
            await trx.commit();
            result = 1;
        }
        catch (error) {
