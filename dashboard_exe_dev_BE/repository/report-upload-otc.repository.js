...
const Insert = async (data) => {
    let result = await db_config_1.default.withSchema("pso").table("t_d_otc").insert(data);
    return result;
};
...
const Trx = async (month, year, userID, dataInsert) => {
    ...
    await db_config_1.default.transaction(async (trx) => {
        try {
            await trx.raw(`SELECT * FROM pso.history_table('${month}', '${year}', '${userID}', null, 't_d_otc')`);
            await trx("pso.t_d_otc").insert(dataInsert);
            await trx.commit();
            result = 1;
        }
        catch (error) {
